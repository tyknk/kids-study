...
子供達学習用/
├── AGENTS.md              ← このファイル
├── まなびのへや.html      ← 統合トップページ（親が設定する落ち着いたデザイン）
└── 算数/
    ├── 掛け算割り算(あまりなし)問題アプリ.html
    └── 余りのある割り算アプリ.html
```

### トップページ（まなびのへや.html）の役割
- 初回起動時に名前入力（必須・スキップ不可）→ LocalStorageに保存
...
`まなびのへや.html` の `SUBJECTS` 配列に追加していく設計になっている。
